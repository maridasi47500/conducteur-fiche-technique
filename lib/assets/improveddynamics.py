...
def parse_lilypond(file_path):
    ...
    notes_data = []
# 4. Extraction des voix
    # Pattern 1 : Ton pattern spécifique (partI, voiceOne...)
    ...
    # Pattern 2 : Si le premier échoue, pattern générique pour variables \relative
    if not voice_blocks:
        voice_blocks = re.findall(r'(\w+)\s*=\s*\\relative\s*[a-g\']*[\s]*\{([^}]*)\}', content, re.DOTALL)

    # Pattern 3 : Si toujours rien, on cherche les blocs \new Voice directs (cas de Beethoven)
    if not voice_blocks:
        # On capture le contenu entre les accolades de \new Voice { ... }
        direct_voice_matches = re.findall(r'\\new Voice\s*\{([^}]*)\}', content, re.DOTALL)
        # On formate pour que ça ressemble à la structure de tes autres blocks (nom, contenu)
        voice_blocks = [(f"Voice_{i+1}", match) for i, match in enumerate(direct_voice_matches)]
