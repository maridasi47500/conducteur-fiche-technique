...
def parse_lilypond(file_path):
    ...
    for d in sorted(all_dynamics, key=lambda x: (x['measure'], x['beat'])):
        print(f"Mesure {d['measure']}, temps {d['beat']:.2f} : {d['type']} (voix {d['voice']})")
...
def parse_notes_in_voice(notes_str, voice_name, beats_per_measure, beat_unit, start_measure):
    patterns = [
        ...
        r'\b((?!(?:[sS]))[a-grR](?:is|es)?[\',]*)([\d\.]*)',               # 4. Simple notes/rests (added 's' for spacers)
        r'\\(p|f|sf|cresc|decresc|[<>!])'                      # 5. DYNAMICS (The new pattern)
    ]
...
if __name__ == "__main__":
    ...
    try:
        notes_extraites = parse_lilypond(fichier)
